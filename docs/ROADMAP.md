...
**Two issues came out of its device test, and they belong to a question this lane has not answered.** #520, Apple FM eats the `<<NL>>` line-break markers — six in, zero out, and measured **not** to be caused by the reframing: 1/12 preserved under the old framing against 3/12 under the new. #521, Normal polish ends every dictation with a period, fragments included. Pierre's own verdict at the end of that session was that Normal polish is not at the level, and that punctuation and Normal mode want work **before** the Smart Modes ship. That is a sequencing claim against this lane's order, and it is not settled here: #437 (long-form paragraph formatting), #439 (the Natural contract broken three ways), #520 and #521 are the four issues it would promote. Decide it at the next pass, not mid-lane.
