...
5. **#437** — faithful long-form paragraph formatting. **Not blocked on anything.** The six paired Dictus/Typeless fixtures were recorded on 2026-08-27, the baseline is measured (0 line breaks in 6 of 6), the contract is decided — *may add whitespace, may never remove words or change their grammar* — and the bars are declared per fixture. The machinery is not missing either: the `<<NL>>` round-trip works and every prompt bans the model from emitting one. **What is left is replacing one clause and measuring it on the Mac.** The `Current status` section of the issue body said otherwise until 2026-09-07; it was eight days stale and it is corrected.
