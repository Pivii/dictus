...
4. **#437** — benchmark faithful long-form paragraph formatting against paired Typeless outputs. **Blocked on Pierre**, not on code: it needs the same audio dictated into both apps before any rule can be inferred, and the issue refuses to validate a formatting rule without those pairs. It is faithful formatting inside Normal polish, no summarising — the deliberate opposite of #523, and the two must not be built in the same round or neither measurement means anything.
5. **#523** — a Smart Mode for the long vocal: paragraphs, and an output allowed to be shorter than the speech. The first Dictus contract that may drop a dictated clause, so it needs its own never-drop classes and its own guardrail band (`PolishGuardrail` refuses below a 0.5 length ratio today, which a real condensation blows straight through). First acceptance step is the paired-output comparison against `List`.
