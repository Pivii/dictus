...
final class PendingDictationTests: XCTestCase {
    ...
    func testSameDocumentMayBeInsertedInto() {
        XCTAssertTrue(pending(documentIdentifier: fieldA).mayInsert(into: fieldA))
    }

    func testDifferentDocumentIsRefused() {
        XCTAssertFalse(pending(documentIdentifier: fieldA).mayInsert(into: fieldB))
    }
    ...
    func testUnknownCurrentDocumentIsRefused() {
        XCTAssertFalse(pending(documentIdentifier: fieldA).mayInsert(into: nil))
    }
    ...
    func testUnknownClaimedDocumentIsRefused() {
        XCTAssertFalse(pending(documentIdentifier: nil).mayInsert(into: fieldA))
    }
    ...
    func testTwoUnknownsAreNotAMatch() {
        XCTAssertFalse(pending(documentIdentifier: nil).mayInsert(into: nil))
    }
    ...
    func testRecoveryInsideTheWindowIsAllowed() {
        ...
        XCTAssertTrue(record.mayRecover(into: fieldA, now: 1_000 + 29))
    }

    func testRecoveryAtTheWindowBoundaryIsAllowed() {
        let record = pending(documentIdentifier: fieldA, claimedAt: 1_000)
        XCTAssertTrue(record.mayRecover(into: fieldA, now: 1_000 + PendingDictation.recoveryWindow))
    }

    func testRecoveryPastTheWindowIsRefused() {
        let record = pending(documentIdentifier: fieldA, claimedAt: 1_000)
        XCTAssertFalse(record.mayRecover(into: fieldA, now: 1_000 + 31))
    }
    ...
    func testRecoveryIntoAnotherFieldIsRefusedEvenImmediately() {
        let record = pending(documentIdentifier: fieldA, claimedAt: 1_000)
        XCTAssertFalse(record.mayRecover(into: fieldB, now: 1_001))
    }
